...
func execute(gIndex int, client *gosdk.Client, acc *EthAccount, generateRandAddress func() sdk.AccAddress, contractAddress string) {
	...
	execMsg := fmt.Sprintf(`{"transfer":{"recipient":"%s"}}`, toAddress.Bech32StringOptimized("ex"))

	ethAddr := acc.GetHexAddress()
	bench32Addr := sdk.AccAddress(ethAddr.Bytes())

	err := sendWasmTx(client, acc.privateKey, acc.GetAccountNumber(), acc.GetNonce(), chainId, "", contractAddress, execMsg, bench32Addr, "1okt")
	if err != nil {
